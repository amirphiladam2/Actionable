...
const AuthScreen = () => {
  ...
  const validateSignupForm = (data) => {
    const errors = validateSigninForm(data).errors;
  ...
  const validateForm = useCallback(() => {
    ...
    if (activeTab === "signin") {
      validationResult = validateSigninForm(signinData);
    } else {
  ...
  const handleGoogleSignIn = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.url) {
        ...
        await WebBrowser.openAuthSessionAsync(result.url, Linking.createURL('/auth/callback'));
