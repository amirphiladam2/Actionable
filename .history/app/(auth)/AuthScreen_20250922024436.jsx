...
import { supabase } from "../../lib/supabase"; // Ensure supabase is correctly initialized
  ...
  const handleGoogleSignIn = useCallback(async () => {
    try {
      ...
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: 'actionable://auth/callback', // Back to the callback route
          queryParams: {
            access_type: 'offline',
            prompt: 'consent',
          },
      ...
      if (data?.url) {
        ...
        await WebBrowser.openAuthSessionAsync(
          data.url, 
          'actionable://auth/callback'  // Match the callback route
        );
      } else {
