...
import { supabase, profileService } from '../lib/supabase'; // Adjust path as needed
...
const EditProfileScreen = ({ navigation }) => {
  ...
  const loadProfile = async () => {
    try {
      ...
      // Get current user
      const { user, error: userError } = await profileService.getCurrentUser();
      if (userError || !user) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { data, error } = await profileService.updateProfile(currentUser.id, {
  ...
  const handleImagePicker = async () => {
    ...
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        // Upload to Supabase
        const { data: uploadData, error: uploadError } = await profileService.uploadAvatar(
          currentUser.id, 
          formData, 
          fileExt
        );
        ...
        const { error: updateError } = await profileService.updateProfile(currentUser.id, {
