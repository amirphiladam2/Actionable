...
// Replace the complex handleGoogleSignIn in useSignInLogic with this simplified version 
// that matches your working LoginScreen logic:
  ...
  return await withRequestGuard(async () => {
    try {
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: 'caloriee://auth', // Using the same redirect as LoginScreen
        },
      });
      ...
      // Simply return the data with URL - let LoginScreen handle WebBrowser
      return { success: true, data };
...
// Your LoginScreen handleGoogleSignIn stays exactly the same:
const handleGoogleSignIn = async () => {
  try {
    const { data, error } = await signInWithGoogle();

    if (error) {
      Alert.alert('Google Sign-In Failed', error.error.message);
      return;
    }
