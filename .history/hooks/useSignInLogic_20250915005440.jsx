...
const DEEP_LINKS = {
  AUTH_CALLBACK: 'https://auth.expo.io/@amirpeter60/actionable',
  RESET_PASSWORD: 'https://auth.expo.io/@amirpeter60/actionable/reset'
};
...
const useSignInLogic = () => {
  ...
  // Google OAuth sign in with proper URL handling
  const handleGoogleSignIn = useCallback(async () => {
    ...
    return await withRequestGuard(async () => {
      try {
        ...
        console.log('OAuth response:', { data: data ? 'received' : 'null', error });
        ...
        if (data?.url) {
          console.log('Opening WebBrowser with URL');
          
          try {
            const result = await WebBrowser.openAuthSessionAsync(
              data.url,
              DEEP_LINKS.AUTH_CALLBACK
            );
            
            console.log('WebBrowser result:', result);
            
            if (result.type === 'success' || result.type === 'dismiss') {
              // For OAuth, success is handled by the auth state change listener
              // Just return success here
              return { success: true, data, webBrowserResult: result };
            ...
            Alert.alert('Error', 'Failed to open sign-in page. Please try again.');
            return { success: false, error: webBrowserError };
          }
        } else {
          console.error('No URL in OAuth response');
          Alert.alert('Error', 'No sign-in URL received. Please try again.');
          return { success: false, error: new Error('No OAuth URL') };
        }
