  ...
  const handleGoogleSignIn = useCallback(async () => {
    return await withRequestGuard(async () => {
      try {
        ...
        if (!data?.url) {
          const noUrlError = new Error('No OAuth URL received from provider');
          ToastUtils.error(ERROR_MESSAGES.GOOGLE_SIGNIN_FAILED, 'Unable to initiate Google sign in');
          return { success: false, error: noUrlError };
        }
        
        const canOpen = await Linking.canOpenURL(data.url);
        
        if (!canOpen) {
          const linkingError = new Error('Cannot open OAuth URL');
          ToastUtils.error(ERROR_MESSAGES.GOOGLE_SIGNIN_FAILED, 'Unable to open Google sign in page');
          return { success: false, error: linkingError };
        }
